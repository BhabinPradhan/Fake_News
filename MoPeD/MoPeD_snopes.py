...
class Config:
    def __init__(self):
        self.base_dir = "/home/odobasia/Downloads/BERT_Project/multimodal_dataset"
        self.img_dir = self.base_dir
        
        # Point to the distinct split files
        self.train_csv = os.path.join(self.base_dir, "train_final.csv")
        self.dev_csv = os.path.join(self.base_dir, "val_final.csv")
        self.test_csv = os.path.join(self.base_dir, "test_final.csv")
        ...
        self.user_self_attention = True
        
        # 4. Training Hyperparameters
        self.batch_size = 16
        self.epochs = 30
        self.save_path = './best_moped_snopes.pth'
...
class MedMMHLDataset(Dataset):
    ...
    def _clean_path(self, path_str):
        """Returns the path string exactly as it appears in your new CSV."""
...
class MoPeD_MMHL(nn.Module):
    def __init__(self, config, tokenizer):
        ...
        # Freeze BERT layers except last 4
    ...
    def forward(self, input_ids, attention_mask, image_features):
        ...
        text_feature_padding = torch.zeros(
            bsz, 412, device=device, dtype=text_cnn_feature.dtype
        )
        ...
        image_feature_padding = torch.zeros(
            bsz, 512, device=device, dtype=image_proj.dtype
        )
    ...
    def fit(self, train_loader, val_loader):
        ...
        # If classes are close to balanced, don't add extra bias.
        class_weights = torch.tensor([1.0, 1.0]).cuda()
        loss = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.02)
    ...
    def evaluate(self, val_loader, epoch, return_report=False):
        ...
        if macro_f1 > self.best_acc:
            ...
            results_file = "moped_snopes_best_report.txt"
            with open(results_file, "w") as f:
                f.write("="*20 + " MoPeD MODEL BEST RESULT " + "="*20 + "\n")
            ...
            print(f"  âœ“ Detailed report saved to {results_file}")
        ...
        if len(misclassified_idx) > 0:
            # We use the config's test_csv path to load the data for lookup
            test_df = pd.read_csv(self.config.test_csv)
            for idx in misclassified_idx[:5]: 
                print(f"ID: {idx} | Truth: {y_true[idx]} | Pred: {y_pred[idx]}")
                # Use .iloc to get the text and image path
                print(f"Content: {test_df.iloc[idx]['content'][:100]}...") 
                print(f"Image Path: {test_df.iloc[idx]['image']}\n")
...
def main():    
    ...
    # Weighted sampling - moderate upweighting only if clearly imbalanced
    ...
    upsample_factor = 1.0 if ratio < 1.2 else np.sqrt(ratio)
    print(f"Upsampling minority class by {upsample_factor:.2f}x")
    ...
    final_f1 = model.evaluate(val_loader, epoch=999)
