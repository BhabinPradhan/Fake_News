...
class SpotFakeModelWeibo(nn.Module):
    def __init__(self):
        super().__init__()
...
class SpotFakeInferenceWeibo:
    def __init__(self, model_path="/home/vongoct/checkpoints/spotfake_weibo.pth", device=None):
        self.device = device if device else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-chinese')
        self.model = SpotFakeModelWeibo().to(self.device)
